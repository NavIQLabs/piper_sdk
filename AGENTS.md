...
Published to PyPI as `piper_sdk`. Current version: 0.6.1.
...
There are no test files, no CI workflows, no linter or formatter config.
Verify changes by running a demo script with CAN hardware attached.
...
- **Bus state check**: Before every read/write, checks
  `bus.state == can.BusState.ACTIVE`. If not ACTIVE, returns error
...
CAN names create separate instances with independent CAN buses.

**Threading model** (started by `ConnectPort()`):
1. **ReadCan thread** — tight loop calling `self.__arm_can.ReadCanMessage()`.
...
2. **CanMonitor thread** — runs at 20Hz (50ms sleep). Calls
   ...
   checks `isOk` status.
3. Both threads are daemon threads, stopped via `threading.Event`.
...
rx_message (can.Message)
  ...
  → self.__UpdateArmStatus(msg)             # copy to thread-safe storage
  → self.__UpdateArmEndPoseState(msg)
  → self.__UpdateArmJointState(msg)
  → ... (14 update methods total)
  → optional: __UpdatePiperFeedbackFK()     # if FK enabled
```

Each `__Update*` method:
1. Acquires a per-field `threading.Lock`
2. Copies decoded data into the interface's cached wrapper object
3. Records a timestamp (`time.time_ns() / 1e9`)
4. Updates FPS counter for that specific message type
