...
Unreleased (fork: crisp 风格柔顺控制器特性)
===========================================

特性
--------

- ✨ feat(controller): `mathx.saturate_torque_rate` 新增力矩变化率饱和
  - 支持标量或逐关节 `delta_tau_max` 限制；通过 `torque_rate_limit` 构造参数在
    `BaseController._send_mit` 中应用（支持标量或逐关节序列，0.0 表示禁用）。
    记录的上一周期力矩为实际发送值（限幅后），并在 `enable()` 时重置
- ✨ feat(controller): `model.py` 新增 `SigmoidFrictionModel`
  - `f(qd) = fp1/(1+exp(-fp2(qd+fp3))) - fp1/(1+exp(-fp2 fp3))`；静止处平滑，
    高速时饱和；`fp3` 平移过渡点
- ✨ feat(controller): `CartesianImpedanceController` 新增目标位姿 EMA 滤波器
  - `target_filter` 参数与 `set_target_filter()`；平动目标逐轴滤波，转动目标在
    SO(3) 上做四元数 slerp（`quat_slerp`），避免万向节锁与角度环绕造成的失真
- ✨ feat(controller): `CartesianImpedanceController` 新增零空间阻尼
  - `nullspace_damping` 参数与 `set_nullspace_damping()`；将关节阻尼投影到 `I - J^+ J`
- ✨ feat(controller): `CartesianImpedanceController` 新增零空间刚度弹簧
  - `nullspace_stiffness` / `nullspace_reference` / `nullspace_max_tau` 参数与
    `set_nullspace_stiffness()` / `set_nullspace_reference()`；通过 `I - J^+ J`
    将冗余自由度拉回 `q_ref`（默认取初始位形），使机械臂在执行笛卡尔任务时保持
    位形；`nullspace_max_tau` 限制投影后的力矩，避免干扰主任务（0.0 不限幅）
- ✨ feat(controller): `CartesianImpedanceController` 新增任务空间误差限幅
  - `error_clip` 参数与 `set_error_clip()`；将进入刚度律的各轴误差限制在
    `±error_clip[i]`，抑制大幅 `set_target` 造成的力矩尖峰（None 表示禁用）
- ✨ feat(controller): `BaseController` 新增输出力矩 EMA
  - `output_torque_filter` 参数；在力矩变化率限幅后对指令力矩做指数平滑
    （0.0 禁用，1.0 直通）
- ✨ feat(controller): `BaseController` 新增关节限位斥力力矩
  - `limit_repulsion_torque` / `limit_repulsion_range` 参数与
    `ArmModel.joint_limit_torque`；在 `safe_range` 内线性增大到 `max_torque`，
    将关节轻柔推离限位，使机械臂在接近限位时仍可控（0.0 禁用）
- ✨ feat(controller): 重写 `CartesianAdmittanceController`，平动/转动解耦导纳
  - 位置采用三自由度线性 MSD，转动采用 SO(3) 四元数指数映射积分；世界系角速度
    左乘到四元数（`exp(ωdt) ⊗ q`，与 crisp 一致）且跨周期持久化，使转动阻尼生效；
    摩擦力支持 Coulomb 偏移列表或 `SigmoidFrictionModel`
...
- ✨ feat(can_comm): 将发送和接收的bus分开，防止资源抢占导致的延时或者发送失败
- ✨ feat(piper_interface_v2): 添加接收和发送消息对象实例
  - 在接收和发送路径中复用`rx_msg`和`tx_msg`，避免重复创建消息对象
- ✨ feat(piper_interface_v2): CAN热路径中按消息类型分发到对应更新处理函数
  - 只调用与当前帧类型匹配的更新处理函数，不再无条件调用全部处理函数
- ✨ feat(piper_interface_v2): 新增`minimal_feedback_mode`与`GetArmStateSnapshot()`
  - 轻量级状态读取路径，避免每次轮询都分配SDK包装对象
- ✨ feat(utils): FPS计数器新增`enabled`开关（`SetPerformanceMetricsEnabled`）

Bug修复
--------
- 🐞 fix(hardware_port): 根据平台创建bus
  - Windows和macOS共享收发bus，Linux使用独立的发送和接收bus
  - 关闭CAN bus时避免重复关闭同一个bus实例
  - 创建bus时禁止接收自身消息与本地回环
- 🐞 fix(hardware_port): CAN bus状态检查改为每50次读取检查一次
- 🐞 fix(interface): 断开连接时停止CanMonitor与FPS线程（线程泄漏修复）
- 🐞 fix(interface): 每个夹爪帧不再重复调用`__CalGripperSDKLimit`两次
- 🐞 fix(utils): 重新启用性能指标时重启FPS计算线程
- 🐞 fix(utils): 计数器禁用期间也注册FPS变量
  - 此前禁用期间添加的变量会被丢弃，重新启用后追踪不到任何数据；现在重新启用后所有已注册变量恢复正常追踪

其它
--------

- 🔧 build(setup.py): 更新版本号至`0.6.2`
- 🔧 perf: 读取CAN消息超时时，读循环让出1ms
