...
Unreleased (fork: crisp-style compliant controller features)
============================================================

Features
--------

- ✨ feat(controller): Adds torque-rate saturation in `mathx.saturate_torque_rate`
  - Scalar or per-joint `delta_tau_max` limit; applied in `BaseController._send_mit`
    via the `torque_rate_limit` constructor arg (scalar or per-joint sequence,
    0.0 = disabled). The recorded previous torque is the actually-sent value
    (post-clamp) and resets on `enable()`.
- ✨ feat(controller): Adds `SigmoidFrictionModel` to `model.py`
  - `f(qd) = fp1/(1+exp(-fp2(qd+fp3))) - fp1/(1+exp(-fp2 fp3))`; smooth at rest,
    saturates at large speeds; `fp3` shifts the transition point
- ✨ feat(controller): Adds target-pose EMA filter to `CartesianImpedanceController`
  - `target_filter` arg + `set_target_filter()`; translational target is filtered
    per-axis, rotational target is slerped on SO(3) (`quat_slerp`) so the filter
    never distorts through gimbal-lock or wrap discontinuities
- ✨ feat(controller): Adds nullspace damping to `CartesianImpedanceController`
  - `nullspace_damping` arg + `set_nullspace_damping()`; projects joint damping
    onto `I - J^+ J`
- ✨ feat(controller): Adds nullspace stiffness spring to `CartesianImpedanceController`
  - `nullspace_stiffness` / `nullspace_reference` / `nullspace_max_tau` args +
    `set_nullspace_stiffness()` / `set_nullspace_reference()`; springs the
    redundant DOFs back toward `q_ref` (defaults to the starting pose) through
    `I - J^+ J`, so the arm holds its configuration during cartesian tasks;
    `nullspace_max_tau` clamps the projected torque so it never fights the main
    task (0.0 = no clamp)
- ✨ feat(controller): Adds task-space error clipping to `CartesianImpedanceController`
  - `error_clip` arg + `set_error_clip()`; bounds each axis of the error fed to
    the stiffness law to `±error_clip[i]`, capping torque spikes from large
    `set_target` steps (None = disabled)
- ✨ feat(controller): Adds output-torque EMA to `BaseController`
  - `output_torque_filter` arg; exponential smoothing of the commanded torque
    applied after torque-rate limiting (0.0 = disabled, 1.0 = passthrough)
- ✨ feat(controller): Adds joint-limit repulsion torque to `BaseController`
  - `limit_repulsion_torque` / `limit_repulsion_range` args + `ArmModel.joint_limit_torque`;
    pushes each joint softly away from its limits (linear ramp up to
    `max_torque` within `safe_range`) so the arm stays controllable near the
    edge (0.0 = disabled)
- ✨ feat(controller): Rewrites `CartesianAdmittanceController` with decoupled
  trans/rot admittance
  - Linear 3-DOF MSD for position + SO(3) quaternion exponential-map rotation
    integration; world-frame angular velocity is left-multiplied onto the
    quaternion (`exp(ωdt) ⊗ q`, matching crisp) and its state persists across
    ticks so rotational damping is active; friction via Coulomb offsets list
    or `SigmoidFrictionModel`
...
- ✨ feat(piper_interface_v2): Adds type-based handler dispatch in the CAN hot path
  - Only the update handler(s) matching the received frame type are invoked, instead of all handlers unconditionally
- ✨ feat(piper_interface_v2): Adds `minimal_feedback_mode` and `GetArmStateSnapshot()`
  - Lightweight state read path that avoids allocating SDK wrapper objects per poll
- ✨ feat(utils): Adds an `enabled` flag to the FPS counter (`SetPerformanceMetricsEnabled`)
...
- 🐞 fix(hardware_port): Creates CAN buses according to the current platform
  ...
  - Disables receiving own messages and local loopback when creating a bus
- 🐞 fix(hardware_port): Throttles the CAN bus state check to every 50th read instead of every read
- 🐞 fix(interface): Stops the CanMonitor and FPS threads on disconnect (thread leak fix)
- 🐞 fix(interface): Avoids calling `__CalGripperSDKLimit` twice per gripper frame
- 🐞 fix(utils): Restarts the FPS calculation thread when performance metrics are re-enabled
- 🐞 fix(utils): Registers FPS variables even while the counter is disabled
  - Previously variables added while disabled were dropped, so re-enabling tracked nothing; now re-enabling resumes tracking of all registered variables

Miscellaneous
---------

- 🔧 build(setup.py): Updates the version to `0.6.2`
- 🔧 perf: Yields 1ms in the read loop on CAN receive timeout
